...
public class CarService {
  ...
  /**
   * Find all cars and filter based on input. If there is no input then return all cars. Otherwise,
   * returns an intersection set of filtered results from repository. Note: requires mileage, year,
   * previous owners ranges. User filter only applied if specified.
   *
