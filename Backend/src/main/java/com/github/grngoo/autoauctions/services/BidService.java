...
public class BidService {
  ...
  /**
   * Search through all bids. Show all bids if no filters in bidDto. Else provide a filtered search
   * of bids. Based on DTO.
   *
  ...
  /**
   * Saves a new bid entity with given a DTO. The bid must be placed within the listing period and
   ...
   * @throws IllegalArgumentException if the bid is placed outside the listing period or is lower
  ...
  private void validateBidAmount(Bid bid, BigDecimal highestBid) {
    if (highestBid != null && highestBid.compareTo(bid.getAmount()) >= 0) {
      throw new IllegalArgumentException(
          "Current bid amount is less than or equal to the highest bid.");
    }
  }
}
